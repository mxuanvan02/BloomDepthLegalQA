...
def run_phase_a(
    ...
    for mode in CFG.refinement.ablation_modes:
        ...
        # Load generator and critic SEQUENTIALLY (L4 can't hold both)
        ...
        qa_pairs, traces = run_ablation(
            ...
            critic_engine=generator.generate,  # Same model for now, swap to critic below
            contexts=contexts,
        ...
        # Unload generator before loading critic (L4 constraint)
        generator.unload()
...
def main():
    ...
    if args.phase in ("b", "all"):
        dataset_by_bloom = group_by_bloom(records)
