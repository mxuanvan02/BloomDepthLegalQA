...
def run_phase_a(
    ...
    for mode in CFG.refinement.ablation_modes:
        ...
        critic_engine = generator.generate  # Default: self-critique

        t0 = time.perf_counter()
        qa_pairs, traces = run_ablation(
            mode=mode,
            generator_engine=generator.generate,
...
def main():
    ...
    if not dataset_path.exists():
        logger.error("Dataset not found: %s (run extract_corpus.py + prepare_data.py first)", dataset_path)
        sys.exit(1)
    ...
    if args.phase in ("b", "all"):
        # Phase B needs QA pairs (generated by Phase A), not the raw context chunks.
        qa_dataset_path = CFG.paths.results / "refinement" / "adaptive_refinement" / "qa_pairs.json"
        
        if not qa_dataset_path.exists():
            # Fallback if specific ablation mode not found
            candidate_paths = list((CFG.paths.results / "refinement").glob("*/qa_pairs.json"))
            if candidate_paths:
                qa_dataset_path = candidate_paths[0]
            else:
                logger.error("Phase A QA pairs not found. Must run phase A first to generate benchmark dataset.")
                sys.exit(1)
                
        logger.info("Loading QA dataset for benchmarking: %s", qa_dataset_path)
